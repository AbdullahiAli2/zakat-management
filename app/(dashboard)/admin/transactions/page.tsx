...
export default function AdminTransactionsPage() {
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader>
          <CardTitle className="text-[#065F46]">Transactions</CardTitle>
        </CardHeader>
        <CardContent className="pt-0 text-black">
          <div className="grid gap-3 md:grid-cols-5">
            <div className="space-y-2">
              <div className="text-sm font-semibold text-black/95">From</div>
              <Input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} onBlur={resetPage} />
            </div>
            <div className="space-y-2">
              <div className="text-sm font-semibold text-black/95">To</div>
              <Input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} onBlur={resetPage} />
            </div>
            <div className="space-y-2">
            ...
            {isFetching && !data ? (
              ...
              <Table>
                ...
                <TableBody>
                  {data.items.map((t) => (
                    <TableRow key={t.id}>
                      <TableCell className="text-black">{new Date(t.date).toLocaleString()}</TableCell>
                      <TableCell className="font-semibold text-black">{formatCurrency(t.amount)}</TableCell>
                      <TableCell className="text-black">
                        <Badge
                          variant={
                            t.type === "ZAKAT_PAYMENT" ? "success" : t.type === "DISTRIBUTION" ? "warning" : "secondary"
                          }
                        >
                          {t.type}
                        </Badge>
                      ...
                      <TableCell className="text-black">{t.reference || "-"}</TableCell>
