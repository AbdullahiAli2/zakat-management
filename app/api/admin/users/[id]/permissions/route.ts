...
import { syncUserPermissionsSchema, toggleUserPermissionSchema } from "@/lib/validation";
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const path = `/api/admin/users/${resolved.id}/permissions`;
  try {
    ...
    const [permissions, links] = await Promise.all([
      ...
      prisma.userPermission.findMany({ where: { userId }, select: { permissionId: true } }),
    ]);
    ...
    await logError({ userId: null, message: e?.message ?? "User permissions fetch failed", stack: e?.stack, path, lineNumber: typeof e?.lineNumber === "number" ? e.lineNumber : null });
    return NextResponse.json({ ok: false, error: "User permissions fetch failed" }, { status: e?.status ?? 500 });
  }
}
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const userId = Number(resolved.id);
    if (!Number.isFinite(userId) || userId <= 0) return NextResponse.json({ ok: false, error: "Invalid user id" }, { status: 400 });
    const validated = parseRequestBody(toggleUserPermissionSchema, await req.json());
    if (!validated.ok) return NextResponse.json(validated.body, { status: validated.status });
    const parsed = validated.data;

    if (parsed.enabled) {
      await prisma.userPermission.upsert({
        where: { userId_permissionId: { userId, permissionId: parsed.permissionId } },
        update: {},
        create: { userId, permissionId: parsed.permissionId },
      });
    } else {
      await prisma.userPermission.deleteMany({
        where: { userId, permissionId: parsed.permissionId },
      });
    }
    ...
    const e = err as { message?: string; stack?: string; status?: number; lineNumber?: number };
    await logError({ userId: null, message: e?.message ?? "Apply user permission failed", stack: e?.stack, path, lineNumber: typeof e?.lineNumber === "number" ? e.lineNumber : null });
    return NextResponse.json({ ok: false, error: "Apply user permission failed" }, { status: e?.status ?? 500 });
  }
}
