...
import { getWalletTransactionHistory } from "@/lib/accounting/wallets";
import { isZodError, zodErrorBody } from "@/lib/parse-request";

export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const resolved = await params;
  const path = `/api/admin/wallets/${resolved.id}/transactions`;
  try {
    ...
    const history = await getWalletTransactionHistory(id, { page, pageSize });
    ...
    await logError({ userId: null, message: e?.message ?? "Fetch wallet transactions failed", path });
    return NextResponse.json({ ok: false, error: e?.message ?? "Fetch wallet transactions failed" }, { status: e?.status ?? 500 });
  }
}
